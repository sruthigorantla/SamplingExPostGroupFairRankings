...
class GroupCountDP:
    def __init__(
        ...
        self.LB, self.UB = self.get_bounds(proportions, p_deviation)
        # Add dummy entry to bounds to ensure that can
        # be indexed as per group id
        self.lower_bounds = [0] + self.LB
        self.upper_bounds = [0] + self.UB
        assert (
            len(proportions) == num_groups
        ), f"Number of bounds do not match the number of groups"
    ...
    def _check_bounds(
        ...
        assert not any(
            [
                a > b
                for (a, b) in zip(self.lower_bounds[1:], self.upper_bounds[1:])
            ]
        )
    ...
    def create_dp_table(
        ...
        print(self.rank_size, self.num_groups,self.lower_bounds,self.upper_bounds)
        # Create a dp table
    ...
    def sample(
        ...
        while sample_count < num_samples:
            ...
            while sum > 0:
                ...
                distribution = [0] * (
                    self.upper_bounds[i] - self.lower_bounds[i] + 1
                )
                ...
                for idx, x in enumerate(
                    ...
                    if sum - x >= 0:
                        distribution[idx] = (
                            self.dp[sum - x, i - 1] / self.dp[sum, i]
                        )
                    else:
                        distribution[idx] = 0
...
def manual_table(
    ...
    for i in range(num_groups):
        somelists.append(
            [x for x in range(lower_bounds[i], upper_bounds[i] + 1)]
        )
